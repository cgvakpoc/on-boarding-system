...
class IDcardController extends Controller
{
    ...
    {
    	$candid_id = IDCard::where('user_id',$id)->first();
    ...
    {
    	$id = $request->id;
    	$card_data = IDCard::where('user_id',$id)->first();
    ...
    {
    	$id_card = IDCard::where('user_id',$id)->first();
        if(!$id_card){
            $error_msg = 'Sorry, Information for id '.$id.' cannot be found';
            error_404(false,$error_msg);
            die;
        }
        $deleted = $id_card->delete();
        $delete_msg = 'ID card details has been deleted';
        success_200(true,$delete_msg);
    }
}
